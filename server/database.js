...
export async function verifyAdminCredentials(db, username, password) {
    ...
    const row = await db.get('SELECT password_hash FROM admins WHERE username = ?', [
...
export async function seedDefaultAdmin(db, opts = {}) {
    ...
    if (force) {
        ...
        console.log(`Database: admin user "${DEFAULT_ADMIN_USER}" saved (use password from project seed).`);
        return;
    }

    const existing = await db.get('SELECT id FROM admins WHERE username = ?', [DEFAULT_ADMIN_USER]);
    if (existing) return;

    await db.run('INSERT INTO admins (username, password_hash) VALUES (?, ?)', [DEFAULT_ADMIN_USER, hash]);
